...
export default function Home(){
  ...
  useEffect(()=>{
    getCars().then(c=>{setCars(c);if(c[0])setCarId(c[0].id)})
    getCategories().then(setCategories)
    getFeatured().then((f)=>setFeatured(Array.isArray(f)?f:[]))
    setBuild(loadBuild())
  },[])

  useEffect(()=>{if(carId)getParts(carId,category||undefined).then((p)=>setParts(Array.isArray(p)?p:[]))},[carId,category])
